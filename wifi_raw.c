...
void __wrap_ppTxPkt(void *buf, uint16_t len)
{
	...
	if (level == 0) {
		...
		struct netif *ifp;
		struct pbuf *pb;
		uint8_t header[] = {'\xff', '\xff', '\xff', '\xff', '\xff', '\xff', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00', '\x00'};
		uint16_t alloc_len;
		...
		memcpy(((uint8_t **)buf)[4], upper_buf, upper_len);
		__real_ppTxPkt(buf);
	}
}
